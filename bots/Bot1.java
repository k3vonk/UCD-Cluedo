...
public class Bot1 implements BotAPI {
        ...
        for (int i = 0; i < tmp.size(); i++) {
            ...
            if (tmp.get(i).contains("questioned")) {
                ...
                if (tmp.get(z).contains("showed")) {
                    ...
                    //System.out.println("USER: " + user + " TOKEN: " + token + " WEAPON: " +
                    // weapon + " ROOM: " + room);
    ...
    private String getClosestRoom(ArrayList<String> rooms) {
        ...
        if (player.getToken().isInRoom()) {
            if (rooms.contains(player.getToken().getRoom().toString())) {
                ...
                ArrayList<Coordinates> bestDoor = calculatePath(
                        player.getToken().getRoom().getDoorCoordinates(
                                Integer.parseInt(getDoor()) - 1),
                        map.getRoom(rooms.get(0)).getDoorCoordinates(0));
                int bestRoom = 0;
                for (int i = 1; i < rooms.size(); i++) {
                    goToRoom = rooms.get(i);
                    ArrayList<Coordinates> tmp = calculatePath(
                            player.getToken().getRoom().getDoorCoordinates(
                                    Integer.parseInt(getDoor()) - 1),
    ...
    private ArrayList<Coordinates> calculatePath(Coordinates s, Coordinates e) {
        AStar pathFinder = new AStar(24, 25);
        ArrayList<Coordinates> path = pathFinder.computeAStar(s, e);
        return path;
    }
    ...
    class AStar {
        ...
        /**
         * Used to compare two nodes based on their F value to find which is the better route to
         * take
         */
        ...
        public AStar(int width, int height) {
            ...
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    ...
                    this.nodes.put(pointS, new Node(point));
                }
            }
        ...
        /**
         * Computes the best path between p1 and p2 (Coordinates)
         *
         * @param p1 from point
         * @param p2 destination point
         */
        public ArrayList<Coordinates> computeAStar(Coordinates p1, Coordinates p2) {
            ...
            /**
             * Was unable to get from hashmap by providing a coordinate so now it stores a String
             * version as key
             */
            ...
            while (!openList.isEmpty()) {
                ...
                /**
                 * If we're trying to find the best path to a door and it finds an easier path
                 * using another door, it returns the easier path
                 *
                 */
                ...
                // Check the nodes at each direction {u,d,l,r}
                for (String direction : mapDirections) {
                    Coordinates adjPoint = map.getNewPosition(currentNode.point, direction);
                    // Make sure we're not moving out of the grid
                    if (!this.isInsideBounds(adjPoint)) {
                        continue;
                    }
                    // Creates a node based on an adjacent point
                    Node adjNode = nodes.get(
                            "(" + adjPoint.getCol() + "," + adjPoint.getRow() + ")");

                    // Check if we are running into a wall and if so, ignore this path
        ...
        /**
         * Works back from destinationNode, finds its parents and finds the way back to home
         *
         * @param destinationNode the final node on our route
         * @return the path from home to destination and vica versa
         */
        private ArrayList<Coordinates> calculatePath(Node destinationNode) {
    ...
    /**
     * Class represents each node within our grid, each point corresponds to a coordinate on the
     * grid
     */
    class Node {
        ...
        public Node parent;
        // Movement cost from starting point to current point
        public int gValue;
        // (Estimated) movement cost from current point to end
        public int hValue;

        // Not very relevant since we've no diagonals.
        private final int MOVEMENT_COST = 10;
        public Node(Coordinates point) {
            this.point = point;
        }
